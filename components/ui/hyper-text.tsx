...
interface HyperTextProps extends Omit<MotionProps, "children"> {
  ...
  /** Whether to preserve original text case (don't force uppercase) */
  preserveCase?: boolean
  /** Whether to use font-mono (defaults to true) */
  monoFont?: boolean
}
...
export function HyperText({
  ...
  useEffect(() => {
    // Don't auto-start if animateOnHover is enabled
  ...
  }, [delay, startOnView, animateOnHover])
    ...
    >
      <AnimatePresence>
        {displayText.map((letter, index) => (
          <motion.span
            key={index}
            className={cn(monoFont ? "font-mono" : "", letter === " " ? "w-3" : "")}
          >
